...
External tools can be configured to be selectable as an LTI resource link during **module item** creation. The **link_selection** placement allows course designers (Admins/Instructors) to use the LTI Deep Linking flow to select an LTI resource from an external tool and associate it with a Canvas module item.
...
For configuration examples and links to the specification, please refer to the LTI Deep Linking documentation. Simply replace the **assignment_selection** text with **link_selection** in the XML (LTI 1.0, 1.1, and 1.2) or JSON (LTI 1.3) examples.
...
- Using [module requirements](https://community.canvaslms.com/t5/Instructor-Guide/How-do-I-add-requirements-to-a-module/ta-p/1131), Course designers can require the students to launch the tool before module progression is allowed.
...
- Tools **cannot synchronize grades and submissions** back to the course gradebook by leveraging LTI grading services. Tools that want this capability should use the assignment_selection placement instead.
...
During [module item creation](https://community.canvaslms.com/t5/Instructor-Guide/How-do-I-add-an-external-URL-as-a-module-item/ta-p/967):
