...
`references/player-collection/` optionally holds the player's own DIM exports,
`destiny-weapon.csv` and `destiny-armor.csv`. These files are gitignored, are not
generated by the rebuild script, and do not appear in `INDEX.md`. Check for them at
the start of every build request:

```
ls references/player-collection/
```

**When the files are present, recommend only gear that appears in them.** This
outranks every other preference. An exotic the player does not own is a shopping list,
not a build. Concretely:

- Weapons come from `destiny-weapon.csv`. A weapon is only recommendable if a row
  exists for it, and a specific roll is only recommendable if those perks appear in
  that row's `Perks 0`..`Perks 17` columns. `Rarity`, `Type`, `Element`, `Ammo`, and
  `Archetype` are the columns worth filtering on.
- Exotic and legendary armor come from `destiny-armor.csv`. `Equippable` is the class,
  `Type` is the slot, and `Tertiary Stat` and the stat columns show what the piece
  actually rolled.
- Armor set bonuses are checkable because pieces carry their set name, so
  `AION Renewal Bond` counts toward the `AION Renewal` entry in
  `armor/set-bonuses.md`. Confirm the player owns enough pieces of the right class
  before building around a 2-piece or 4-piece bonus.
- Subclass abilities, aspects, fragments, armor mods, and artifact perks are not items
  and do not appear in these exports. Never restrict those to the collection.
- Rows repeat when the player holds several copies, so dedupe by name before counting.
- The export is a snapshot. If the player says they own something the file lacks,
  take their word for it and say which file disagreed.
- When the goal genuinely needs a piece the player does not own, build the best legal
  loadout from what they have, then name the missing item once as an upgrade path.

Filtering is easiest through the csv module, since the perk columns contain commas
and quotes:

```
python3 - <<'EOF'
import csv
for row in csv.DictReader(open('references/player-collection/destiny-armor.csv')):
    if row['Rarity'] == 'Exotic' and row['Equippable'] == 'Warlock':
        print(row['Name'], '|', row['Type'])
EOF
```

**When the files are absent, ignore all of the above** and build from the full
references as usual. Add one line at the end of the answer telling the player they can
narrow future builds to their own gear: in DIM (app.destinyitemmanager.com) open
Settings, find the spreadsheet export, download the Weapons and Armor CSVs, and save
them as `references/player-collection/destiny-weapon.csv` and `destiny-armor.csv`.
Mention this once, at the end, and never repeat it in the same answer.
...
**Owned gear only, when the collection is present.** If
`references/player-collection/` holds the player's DIM exports, every weapon and every
armor piece in the build has to appear in them. See "The player's collection".
...
0. **Check the collection.** `ls references/player-collection/`. If the exports are
   there, every weapon and armor piece in the build must come from them; if they are
   not, build freely and close the answer with the one-line DIM export tip.
...
8. **Run the legality check.** Walk the build once more against every rule in
   ...
   exotic armor piece, at most one exotic weapon, one artifact, mods in the right
   slots, and every weapon and armor piece present in the collection when one is
   loaded.
