...
External tools can be configured to appear as buttons in the rich content editor. The **editor_button** placement allows many users to use the LTI Deep Linking flow to select resources from an external tool and embed them in the [Canvas Rich Content Editor](https://community.canvaslms.com/t5/Canvas-Basics-Guide/What-is-the-New-Rich-Content-Editor/ta-p/12) (RCE).
...
- Grades cannot be applied to editor content; use the assignment_selection placement if grades are desired.
- Requires UI interactions to generate content. To generate content server-to-server, use of Canvas API is required.
