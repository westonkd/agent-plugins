...
Value conforms with Caliper 1.1, i.e. [http://purl.imsglobal.org/ctx/caliper/v1p1](http://purl.imsglobal.org/ctx/caliper/v1p1)
...
- Each event has a @context attribute whose value is [http://purl.imsglobal.org/ctx/caliper/v1p1](http://purl.imsglobal.org/ctx/caliper/v1p1)
...
edApp.id is represented as a customer Canvas domain URL e.g : [http://oxana.instructure.com/](http://oxana.instructure.com/) or a default URL: [www.canvaslms.com](http://www.canvaslms.com) depending on the producer of the event and the application end user accessing Canvas from.
...
edApp.id = DEFAULT_URL = '[www.canvaslms.com](http://www.canvaslms.com)' when an event is triggered by an automated job run by software application. Typically you will not see request, session and hostname details in those events.
